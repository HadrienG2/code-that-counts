...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let add_benchmarks = |v: &mut Vec<(&'static str, CounterBox)>,
                          s: &[(&'static str, fn(u64) -> u64)]| {
        v.extend(
            s.iter()
                .copied()
                .map(|(s, f)| (s, Box::new(f) as CounterBox)),
        );
    };
    add_benchmarks(
        &mut benchmarks,
    ...
    #[cfg(target_feature = "avx2")]
    {
        add_benchmarks(
            ...
            &[
                ...
                ("thread_rayon", |target| {
                    counter::thread_rayon(target, counter::narrow_u8_tuned)
                }),
            ],
        );
    ...
    for (group, mut counter) in benchmarks {
